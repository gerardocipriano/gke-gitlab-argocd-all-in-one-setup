...
gitops_create_content_configmap() {
    ...
    for app_dir in "${manifests_dir}"/*/; do
        ...
        cp "${app_dir}"*.yaml "${repo_root}/manifests/${app_name}/" 2>/dev/null || true
        cp "${app_dir}"*.yml "${repo_root}/manifests/${app_name}/" 2>/dev/null || true
    done
    ...
    if [[ -d "${inventory_dir}" ]]; then
        ...
        cp "${inventory_dir}"/*.yaml "${repo_root}/inventory/" 2>/dev/null || true
        cp "${inventory_dir}"/*.yml "${repo_root}/inventory/" 2>/dev/null || true
    fi
